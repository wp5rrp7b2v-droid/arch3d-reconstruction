"""T-024 Rufu Master builder / inspector / review renderer."""
...
def validate_params(d):
    p=pmap(d)
    assert d["component_id"]=="CMP-FRAME-RUFU-001"
    assert d["master_id"]=="CMP-FRAME-RUFU-001_MASTER"
    assert d["master_version"]=="V001"
    assert d["geometry_mode"]=="BOUNDED_LONG_MEMBER_OUTER_ENVELOPE"
    assert d["role_variants"]==["UPPER","LOWER","NE","SE","SW","NW"]
    assert d["role_geometry_policy"]=="ASSEMBLY_OR_PLACEMENT_ROLE_ONLY / SHARED_CANONICAL_BODY"
    assert d["sample_to_instance_mapping"]=="UNKNOWN"
    assert d["measurement_table_row_count"]==8 and d["complete_measured_sample_count"]==6 and d["unmeasured_table_row_count"]==2
    assert len(d["measured_section_samples_mm"])==6
    assert p["historical_full_length_mm"]["value"] is None
    assert p["canonical_reference_length_mm"]["historical_claim"] is False
    assert p["exact_plan_angle_deg"]["value"] is None
    assert d["angle_rule"]["default_to_45_deg_allowed"] is False
    assert d["groove_boundary"]["status"]=="DIRECT_EXISTENCE / GEOMETRY_DEFERRED"
    assert d["groove_boundary"]["stage1_cut_geometry"] is False
    assert d["visual_reference_gate"]["rufu_pass_decision"]=="D-082"
...
def make_body(d,p,role):
    ...
    mesh=bpy.data.meshes.new("CMP_FRAME_RUFU_001_BODY_MESH"); mesh.from_pydata(verts,[],faces); mesh.update()
    col=bpy.data.collections.new("MASTER__CMP_FRAME_RUFU_001"); bpy.context.scene.collection.children.link(col)
    obj=bpy.data.objects.new("MASTER__CMP_FRAME_RUFU_001__BODY",mesh); col.objects.link(obj)
    obj["component_id"]=d["component_id"]; obj["master_id"]=d["master_id"]; obj["role"]=role
    obj["sample_to_instance_mapping"]="UNKNOWN"; obj["exact_plan_angle_state"]="UNKNOWN_NULL_DO_NOT_LOCK"
    ...
    obj["angle_hard_fail"]=d["angle_rule"]["hard_fail_id"]
    return obj
...
def body_payload(obj):
    ...
    return {"name":obj.name,"vertex_count":len(verts),"face_count":len(faces),
            ...
            "primitive":"closed_rectangular_bounded_outer_envelope_reference","unsupported_detail_count":0,"groove_cut_count":0,
...
def camera(target,pos,scale):
    data=bpy.data.cameras.new("T024_CAMERA"); c=bpy.data.objects.new("T024_CAMERA",data); bpy.context.scene.collection.objects.link(c)
...
def render_views(obj,p,review):
    ...
    obj.data.materials.append(emissive("T024_BODY",0.58))
...
def text_page(path,title,lines):
    clear_scene(); s=setup_scene(); s.render.resolution_x=1200; s.render.resolution_y=900; dark=emissive("T024_TEXT",0.08)
...
def render_summaries(d,review):
    review=Path(review)
    text_page(review/"DIMENSION_PARAMETER_SUMMARY.png","T-024 RUFU / DIMENSION PARAMETER SUMMARY",[
        "Canonical section: 330.5 x 187.2 mm / measured family mean",
        "Table rows: 8 / complete measured samples: 6 / unmeasured: 2",
        "Roles: UPPER LOWER NE SE SW NW / shared canonical body",
        "Canonical reference length: 1000 mm / NON-HISTORICAL",
        "Historical full length: UNKNOWN / exact plan angle: UNKNOWN",
        "Groove: existence DIRECT / geometry DEFERRED"
    ])
    text_page(review/"EVIDENCE_UNCERTAINTY_SUMMARY.png","T-024 RUFU / EVIDENCE + UNCERTAINTY",[
        "Source: SRC-ZG-WF-001 / PDF p83-84 / printed p68-69 / Table 2-40 / Fig.2-42",
        "Physical instances: 8 / sample-to-instance mapping UNKNOWN",
        "UPPER/LOWER and four corners are roles, not geometry variants",
        "No baked 45-degree assumption; orientation is assembly-owned",
        "Groove exact dimensions/position/contour remain UNKNOWN",
        "D-076 visual gate PASS for Rufu under D-082",
...
def build(params,role,asset,semantic,review=None):
    ...
    assert role in ("UNASSIGNED","UPPER","LOWER","NE","SE","SW","NW")
    ...
    sem={"task":"T-024","component_id":d["component_id"],"master_id":d["master_id"],"master_version":d["master_version"],
         "role":role,"blender_version":bpy.app.version_string,"geometry_mode":d["geometry_mode"],
         "resolved_parameters":{k:v["value"] for k,v in p.items()},"sample_to_instance_mapping":d["sample_to_instance_mapping"],
         "role_geometry_policy":d["role_geometry_policy"],"interface_contract":d["interface_contract"],
         "assembly_semantics":d["assembly_semantics"],"groove_boundary":d["groove_boundary"],"angle_rule":d["angle_rule"],
         "historical_state":d["historical_state"],"assembly_length_rule":d["assembly_length_rule"],"body":body,
    ...
    if review: render_views(obj,p,review); render_summaries(d,review)
    print("RUFU_BUILD_OK",role,sem["semantic_geometry_signature"])

def inspect(expected,asset,output):
    e=json.loads(Path(expected).read_text(encoding="utf-8")); obj=bpy.data.objects.get("MASTER__CMP_FRAME_RUFU_001__BODY")
    ...
    assert obj.get("sample_to_instance_mapping")=="UNKNOWN"; assert obj.get("exact_plan_angle_state")=="UNKNOWN_NULL_DO_NOT_LOCK"
