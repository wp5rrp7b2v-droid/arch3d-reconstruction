"""T-003 Part C: validate the returned asset without repairing it."""
...
if bpy.app.version[:2] != (3, 6):
    fail(f"LOCAL_BLENDER_MUST_BE_3_6;ACTUAL={bpy.app.version_string}", 30)

missing = [name for name in REQUIRED_OBJECTS if bpy.data.objects.get(name) is None]
if missing:
    fail("MISSING_OBJECTS=" + ",".join(missing), 31)
...
for name in REQUIRED_OBJECTS:
    ...
    if obj.type != "MESH" or len(obj.data.vertices) == 0:
        invalid.append(name)
if invalid:
    fail("INVALID_GEOMETRY=" + ",".join(invalid), 32)

scene = bpy.context.scene
poc_value = scene.get("ARCH3D_CLOUD_POC")
cloud_version = scene.get("ARCH3D_CLOUD_BLENDER_VERSION")

if poc_value != "P0_2_V002":
    fail(f"ARCH3D_CLOUD_POC_UNREADABLE_OR_INVALID={poc_value!r}", 33)
if not isinstance(cloud_version, str) or not cloud_version:
    fail(f"ARCH3D_CLOUD_BLENDER_VERSION_UNREADABLE={cloud_version!r}", 34)
