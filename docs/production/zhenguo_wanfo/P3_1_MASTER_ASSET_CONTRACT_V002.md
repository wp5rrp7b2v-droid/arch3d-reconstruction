# P3.1 Canonical Master Asset Contract V002

Status: **LOCKED / PRODUCT OWNER APPROVED / D-036**  
Date: 2026-09-13  
Phase/Gate: `P3 / P3.1｜Component Master & Variant Library`  
Qualification baseline: `T-010 APPROVED / D-033`  
Previous contract: `P3_1_MASTER_ASSET_CONTRACT_V001 / D-034`  
Supersession: **V002 SUPERSEDES V001 FOR ALL FUTURE MASTER PRODUCTION**  
Change type: **FIELD-MAPPING CORRECTION ONLY**  
Scope: **6 MASTER_REQUIRED components only**

## 0. V002 Change Control

V002 继承 V001 的全部资产规则、历史边界、LOD、Variant、审核、Registry 与 reproducibility 要求，仅修正 `CMP-LUDOU-COLUMN-001｜柱头栌斗` 的两个字段映射：

- `bottom_width = 327.1 mm`（面阔下宽）；
- `top_depth = 446.3 mm`（进深总深）。

V001 曾错误写成 `top_depth = 327.1`、`bottom_width = 446.3`。该错误属于已核实证据的字段转录错误，不是新证据或历史解释变化。

权威依据：`docs/evidence/zhenguo_wanfo/P1_2_DIRECT_PAGE_REVIEW_V001.md` D-007，PDF p62–63 / 印刷 p47–48，表2-26、2-27。

V002 不改变 T-011、已批准 `CMP-COLUMN-001` Master V001、任何 evidence classification、其他五类构件参数或 P2 frozen baseline。
...
Authorized observed inputs（V002 corrected mapping）：
...
本合同由 Product Owner 于 2026-09-13 明确批准并锁定（D-036），是 P3.1 后续 Master 生产的 authoritative Contract。

V002：

- **不等于 P3.1 PASS**；
- **不改变 D-035 已批准的柱 Master / T-011**；
- **不自动授权剩余五类 Blender 生产**；
- 只修正柱头栌斗字段映射并恢复剩余五类的任务规划资格。

后续剩余五类建议按风险与构件族拆分：

1. 斗类 batch：柱头栌斗 + 单向长开斗 + 交互斗；
2. 六椽栿 batch：下六椽栿 + 上六椽栿。

每个 batch 仍需独立 Task Contract 和明确授权。