...
| P1.2｜正式证据包建立 | 对锁定案例建立可追溯资料包，至少覆盖年代/身份、总体尺寸、平面柱网、立面屋顶、主体木构、关键构件及图像/测绘/考古或修缮资料 | **READY_FOR_APPROVAL / PASS_RECOMMENDED** | `SRC-ZG-WF-001`完整精细测绘报告已直接核读；Direct Page Review V001 + Parameter Candidate Matrix V002 + Gate Review 已完成。10/10 验收域通过。按治理规则等待 Product Owner Gate Approval；批准前仍不计正式 PASS。 |
| P1.3｜证据分级与可复原性 Go / No-Go | 将关键信息分为已证实 / 高可信推断 / 合理补全 / 未知，并判断是否足以进入正式参数化与 3D 复原 | **LOCKED** | P1.2 获 Product Owner 正式批准后解锁；P1.3 PASS 前不得进入正式 3D 生产。 |

**P1 Gate Progress：2 / 4 PASS；P1.2 已完成 Gate Review，等待 Product Owner 批准。**
...
仍未解决的963柱高、统一Z基准、角部45°精确节点、逐构件原构/后世替换等问题不视为 P1.2 资料包缺失，转交 P1.3 正式分级。
