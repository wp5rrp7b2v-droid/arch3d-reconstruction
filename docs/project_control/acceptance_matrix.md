...
| P1.2｜正式证据包建立 | 对锁定案例建立可追溯资料包，至少覆盖年代/身份、总体尺寸、平面柱网、立面屋顶、主体木构、关键构件及图像/测绘/考古或修缮资料 | **IN PROGRESS** | 当前正式对象：平遥镇国寺万佛殿。关键事实必须能回溯来源；证据不足项不得伪装为事实；当前仍不进入正式 3D 生产。 |
| P1.3｜证据分级与可复原性 Go / No-Go | 将关键信息分为已证实 / 高可信推断 / 合理补全 / 未知，并判断是否足以进入正式参数化与 3D 复原 | **LOCKED** | P1.2 完成后解锁；P1.3 PASS 前不得进入正式 3D 生产。 |

**P1 Gate Progress：2 / 4 PASS。**
...
P1.0 / P1.1 属于研究与项目判断，不创建 Codex T-###。P1.2 当前仍以 ChatGPT 研究、资料获取与证据登记为主；只有出现实际工程执行任务时再创建 T 编号。
